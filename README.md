...
This is a Python-based **Cross-Site Scripting (XSS) vulnerability scanner** designed for penetration testers, bug bounty hunters, and security researchers.  
...
```bash
git clone https://github.com/zeroSLick/XSS_SCANEER.git
cd zeroSlick
```
...
### Run basic scan
```bash
python XSS.py -u http://target.com
```

### Save results to a file
```bash
python XSS.py -u http://target.com -o xss-results.txt
```

### Use multiple threads (default: 5)
```bash
python XSS.py -u http://target.com --threads 10
```

### Scan subdomains
```bash
python XSS.py -u http://target.com --subs
```

### Scan for DOM-based XSS (JS sinks)
```bash
python XSS.py -u http://target.com --sinks
```

### Use headless browser for DOM XSS
```bash
pythonXSS.py -u http://target.com --dom
```
...
### Basic scan
```bash
python XSS.py -u http://testphp.vulnweb.com
```

### Scan and save results
```bash
python XSS.py -u http://testphp.vulnweb.com -o results.txt
```

### Scan with 10 threads
```bash
pythonXSS.py -u http://testphp.vulnweb.com --threads 10
```

### Scan subdomains and detect DOM XSS
```bash
python XSS.py -u http://example.com --subs --sinks --dom
```
